...
def train_multi_3D(Flags):
    ...
    for i, img in enumerate(images):
        ...
        original_points_set = torch.tensor(img1)[indices[:, 0], indices[:, 1], indices[:, 2]]
        original_points_set = original_points_set.float().reshape(num_points, 1)
        indices = torch.Tensor(indices).to(DEVICE)
        pts_glb_all2 = Coord.idx2glb(indices)
        ...
        for step in trange(start, N_steps):
            ...
            if step % Flags.i_print == 0:
                with torch.no_grad():
                    ...
                    output_recon[indices[:, 0], indices[:, 1],indices[:, 2]] = voxel
                    loss_avg = img2mse(output_recon, img)
                    ...
                    if step % (N_steps-1) == 0:
                        ...
                        for j in range(output_recon.shape[0]):
                            ...
                            tifffile.imwrite(
                                os.path.join(Flags.basedir, Flags.expname, f"decompression", f'Decompression_{save_index:03d}.tif'),
